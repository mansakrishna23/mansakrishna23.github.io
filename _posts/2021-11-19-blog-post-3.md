...
```
Our function, as shown above, connects to our database with the `get_message_db` function, selects `n` random handles and corresponsing messages from our database using the `f'SELECT handle, message FROM messages ORDER BY RANDOM() LIMIT {n}'` SQL command, closes the connection to the database, and finally returns the sample of `n` random messages and handles. That being done, we can write our `view` template to display the random sample of messages we have extracted! 
